...
from utils.util import make_size_equal, normalize_score
...
def efficacy_test(
    ...
    mlefficacy = MLEfficacy(
        model_name=model_name, task=task, preprocess_in_pipeline=True
    )
...
if __name__ == "__main__":
    ...
    df_test = pd.read_csv("./data/adult/k1000/test.csv")
    df_synth = pd.read_csv("./synth.csv")
    score = efficacy_test(
        realdata=df_test,
        fakedata=df_synth,
        target_name="income",
        scorer=[accuracy_score, f1_score],
        model_name="mlp",
    )
    print(score)
